...
EXAMPLES = """
- code: 'avi_stringgroup controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_stringgroup'
description: "Adds/Deletes StringGroup configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'stringgroup/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('stringgroup', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
