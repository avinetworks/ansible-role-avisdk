...
EXAMPLES = """
- code: 'avi_cloudproperties controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_cloudproperties'
description: "Adds/Deletes CloudProperties configuration from Avi Controller."
"""
...
short_description: CloudProperties Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'cloudproperties/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('cloudproperties', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
