...
EXAMPLES = """
- code: 'avi_hardwaresecuritymodulegroup controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_hardwaresecuritymodulegroup'
description: "Adds/Deletes HardwareSecurityModuleGroup configuration from Avi Controller."
"""
...
short_description: HardwareSecurityModuleGroup Configuration
description:
    ...
    try:
        ...
        if state == 'absent':
            try:
                rsp = api.delete_by_name(
                    'hardwaresecuritymodulegroup', name,
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'hardwaresecuritymodulegroup/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('hardwaresecuritymodulegroup', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
