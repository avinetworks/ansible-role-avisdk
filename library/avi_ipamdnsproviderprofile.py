...
EXAMPLES = """
- code: 'avi_ipamdnsproviderprofile controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_ipamdnsproviderprofile'
description: "Adds/Deletes IpamDnsProviderProfile configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'ipamdnsproviderprofile/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('ipamdnsproviderprofile', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
