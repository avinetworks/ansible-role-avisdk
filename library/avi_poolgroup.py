...
EXAMPLES = """
- code: 'avi_poolgroup controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_poolgroup'
description: "Adds/Deletes PoolGroup configuration from Avi Controller."
"""
...
short_description: PoolGroup Configuration
description:
...
options:
    ...
    deployment_policy_ref:
        description:
            - When setup autoscale manager will automatically promote new pools into production when deployment goals are met. object ref PoolGroupDeploymentPolicy.
        type: string
    description:
        description:
            - Not present.
    ...
    priority_labels_ref:
        description:
            - UUID of the priority labels. If not provided, pool group member priority label will be interpreted as a number with a larger number considered higher priority. object ref PriorityLabels.
        type: string
    tenant_ref:
        description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'poolgroup/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('poolgroup', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
