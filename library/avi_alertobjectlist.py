...
EXAMPLES = """
- code: 'avi_alertobjectlist controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_alertobjectlist'
description: "Adds/Deletes AlertObjectList configuration from Avi Controller."
"""
...
short_description: AlertObjectList Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'alertobjectlist/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('alertobjectlist', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
