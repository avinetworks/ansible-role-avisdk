...
EXAMPLES = """
- code: 'avi_cluster controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_cluster'
description: "Adds/Deletes Cluster configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'cluster/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('cluster', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
