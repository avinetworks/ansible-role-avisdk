...
EXAMPLES = """
- code: 'avi_alertemailconfig controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_alertemailconfig'
description: "Adds/Deletes AlertEmailConfig configuration from Avi Controller."
"""
...
short_description: AlertEmailConfig Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'alertemailconfig/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('alertemailconfig', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
