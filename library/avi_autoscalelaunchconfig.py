...
EXAMPLES = """
- code: 'avi_autoscalelaunchconfig controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_autoscalelaunchconfig'
description: "Adds/Deletes AutoScaleLaunchConfig configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'autoscalelaunchconfig/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('autoscalelaunchconfig', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
