...
EXAMPLES = """
- code: 'avi_networkruntime controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_networkruntime'
description: "Adds/Deletes NetworkRuntime configuration from Avi Controller."
"""
...
short_description: NetworkRuntime Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'networkruntime/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('networkruntime', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
