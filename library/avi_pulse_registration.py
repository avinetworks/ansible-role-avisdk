...
def main():
    ...
    api = ApiSession.get_session(api_creds.controller, api_creds.username, password=api_creds.password,api_version=api_creds.api_version,timeout=api_creds.timeout,          tenant=api_creds.tenant,
    ...
    if state == 'present':
        ...
        if resp.json().get("registration_status") == "ALBSERVICES_DEREGISTERED":
            if not check_mode:
                ...
                data= {"name":name,"description":description,"email":email,"account_id":account_id}
        ...
        if reg and rsp.status_code>300:
            ...
            if  optins == 'present':
                ...
                response=api.get(path,tenant=tenant, tenant_uuid=tenant_uuid,api_version=api_version)
                path= "albservicesconfig"
                data = dict()
        ...
        if resp.json().get("registration_status")=="ALBSERVICES_DEREGISTERED":
            ...
            reg=False
        if reg and rsp.status_code > 300:
            return module.fail_json(msg='Failed: %s' %
                                        rsp.text)
        else:
