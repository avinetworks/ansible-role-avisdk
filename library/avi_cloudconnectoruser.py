...
EXAMPLES = """
- code: 'avi_cloudconnectoruser controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_cloudconnectoruser'
description: "Adds/Deletes CloudConnectorUser configuration from Avi Controller."
"""
...
short_description: CloudConnectorUser Configuration
description:
...
def main():
    try:
        ...
        sensitive_fields = set(['private_key'])
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'cloudconnectoruser/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('cloudconnectoruser', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
