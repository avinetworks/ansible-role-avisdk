...
EXAMPLES = """
- code: 'avi_networkprofile controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_networkprofile'
description: "Adds/Deletes NetworkProfile configuration from Avi Controller."
"""
...
short_description: NetworkProfile Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'networkprofile/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('networkprofile', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
