...
EXAMPLES = """
- code: 'avi_application controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_application'
description: "Adds/Deletes Application configuration from Avi Controller."
"""
...
short_description: Application Configuration
description:
...
options:
    ...
    virtualservice_refs:
        description:
            - Not present. object ref VirtualService.
        type: string
'''
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'application/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('application', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
