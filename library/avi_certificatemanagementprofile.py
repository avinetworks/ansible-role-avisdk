...
EXAMPLES = """
- code: 'avi_certificatemanagementprofile controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_certificatemanagementprofile'
description: "Adds/Deletes CertificateManagementProfile configuration from Avi Controller."
"""
...
short_description: CertificateManagementProfile Configuration
description:
    ...
    try:
        ...
        if state == 'absent':
            try:
                rsp = api.delete_by_name(
                    'certificatemanagementprofile', name,
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'certificatemanagementprofile/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('certificatemanagementprofile', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
