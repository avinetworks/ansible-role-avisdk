...
EXAMPLES = """
- code: 'avi_jobentry controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_jobentry'
description: "Adds/Deletes JobEntry configuration from Avi Controller."
"""
...
short_description: JobEntry Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'jobentry/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('jobentry', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
