...
EXAMPLES = """
- code: 'avi_role controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_role'
description: "Adds/Deletes Role configuration from Avi Controller."
"""
...
short_description: Role Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'role/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('role', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
