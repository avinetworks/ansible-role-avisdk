...
EXAMPLES = """
- code: 'avi_vipgnameinfo controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_vipgnameinfo'
description: "Adds/Deletes VIPGNameInfo configuration from Avi Controller."
"""
...
short_description: VIPGNameInfo Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'vipgnameinfo/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('vipgnameinfo', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
