...
EXAMPLES = """
- code: 'avi_snmptrapprofile controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_snmptrapprofile'
description: "Adds/Deletes SnmpTrapProfile configuration from Avi Controller."
"""
...
short_description: SnmpTrapProfile Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'snmptrapprofile/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('snmptrapprofile', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
