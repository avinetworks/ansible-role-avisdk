...
EXAMPLES = """
- code: 'avi_networksecuritypolicy controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_networksecuritypolicy'
description: "Adds/Deletes NetworkSecurityPolicy configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'networksecuritypolicy/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('networksecuritypolicy', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
