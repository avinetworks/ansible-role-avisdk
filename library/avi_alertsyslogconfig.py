...
EXAMPLES = """
- code: 'avi_alertsyslogconfig controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_alertsyslogconfig'
description: "Adds/Deletes AlertSyslogConfig configuration from Avi Controller."
"""
...
short_description: AlertSyslogConfig Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'alertsyslogconfig/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('alertsyslogconfig', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
