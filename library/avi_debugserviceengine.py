...
EXAMPLES = """
- code: 'avi_debugserviceengine controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_debugserviceengine'
description: "Adds/Deletes DebugServiceEngine configuration from Avi Controller."
"""
...
short_description: DebugServiceEngine Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'debugserviceengine/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('debugserviceengine', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
