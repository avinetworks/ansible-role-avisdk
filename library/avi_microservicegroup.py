...
EXAMPLES = """
- code: 'avi_microservicegroup controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_microservicegroup'
description: "Adds/Deletes MicroServiceGroup configuration from Avi Controller."
"""
...
short_description: MicroServiceGroup Configuration
description:
...
options:
    ...
    service_refs:
        description:
            - Configure MicroService(es) object ref MicroService.
        type: string
    tenant_ref:
        description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'microservicegroup/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('microservicegroup', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
