...
EXAMPLES = """
- code: 'avi_tenant controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_tenant'
description: "Adds/Deletes Tenant configuration from Avi Controller."
"""
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'tenant/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('tenant', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
