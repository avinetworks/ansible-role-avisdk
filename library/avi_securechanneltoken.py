...
EXAMPLES = """
- code: 'avi_securechanneltoken controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_securechanneltoken'
description: "Adds/Deletes SecureChannelToken configuration from Avi Controller."
"""
...
short_description: SecureChannelToken Configuration
description:
    ...
    try:
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'securechanneltoken/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('securechanneltoken', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
