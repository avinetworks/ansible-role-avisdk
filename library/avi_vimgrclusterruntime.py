...
EXAMPLES = """
- code: 'avi_vimgrclusterruntime controller=10.10.25.42 username=admin '
            ' password=something'
            ' state=present name=sample_vimgrclusterruntime'
description: "Adds/Deletes VIMgrClusterRuntime configuration from Avi Controller."
"""
...
short_description: VIMgrClusterRuntime Configuration
description:
    ...
    try:
        module = AnsibleModule(
            argument_spec=dict(
                ...
                datacenter_managed_object_id=dict(
                    type='str',
                    ),
        ...
        if existing_obj:
            ...
            if changed:
                ...
                rsp = api.put(
                    'vimgrclusterruntime/%s' % obj_uuid, data=obj,
                    tenant=tenant, tenant_uuid=tenant_uuid)
        else:
            changed = True
            rsp = api.post('vimgrclusterruntime', data=obj,
                           tenant=tenant, tenant_uuid=tenant_uuid)
        if rsp is None:
